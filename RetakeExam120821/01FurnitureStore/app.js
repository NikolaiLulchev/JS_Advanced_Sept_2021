...
function solve() {
    ...
    function addFurniture(e) {
        ...
        if (modelField.value != '' && descriptionField.value != '' && yearValue > 0 && priceValue > 0) {
            ...
            trHide.innerHTML = `<td>Year: ${yearValue}</td><td colspan = "3">Description: ${descriptionField.value}</td>`;

            furnitureList.appendChild(tr);
            furnitureList.appendChild(trHide);

            const moreInfoBtn = tr.querySelectorAll('button');
            moreInfoBtn[0].addEventListener('click', showMoreInfo);
            moreInfoBtn[1].addEventListener('click', buyFurniture);
    ...
    function buyFurniture(e) {
        ...
        trHide.parentElement.removeChild(trHide);
